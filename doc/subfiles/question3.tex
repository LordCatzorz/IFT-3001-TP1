...
Nous aurons alors la définition de récurence suivante:
\[
  C\pars{3^k - 1}=
  \begin{cases}
    1 & \text{si } k \leq 0 \\
    1 + 3 \cdot C\pars{\pars{3^k - 1} - \floor{\frac{\pars{3^k - 1}+1}{3}}} & \text{si } k > 1 \\
  \end{cases}
\]
\begin{deriv}
    C\pars{n}
    \<= 
    \commentaire{En supposant $n = 3^k - 1$}
    C\pars{3^k - 1}
    \<=
    \commentaire{Définition de $C\pars{3^k - 1}$ via la définition de $C\pars{n}$}
    1 + 3 \cdot C\pars{n-\floor{\frac{n+1}{3}}}
\end{deriv}
