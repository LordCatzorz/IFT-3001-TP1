...
\begin{deriv}
  ...
  %2^k \cdot C\pars{3^{0}} + \sum\limits_{i=k-(k-1)}^{k}\pars{i3^i}
  \<=
  \commentaire{Simplification}
  ...
  \<=
  \commentaire{Simplification}
  3 \cdot \pars{n-2^{log_3(n)}}
  \<=
  \commentaire{Règle des log}
  3 \cdot \pars{n-2^{log_3(2)log_2(n)}}
  \<=
  \commentaire{Règle des exposants}
  3 \cdot \pars{n-\pars{2^{log_2(n)}}^{log_3(2)}}
  \<=
  \commentaire{Simplification}
  3 \cdot \pars{n-n^{log_3(2)}}
  \<\approx
  \commentaire{Approximation du $log_3 2$}
  3 \cdot \pars{n-n^{0.63093}}
\end{deriv}
