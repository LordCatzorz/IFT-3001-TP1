...
\begin{deriv}
  ...
  2^k \cdot C\pars{3^{k-k}} + \sum\limits_{i=k-(k-1)}^{k}\pars{i3^i}
  \<=
  \commentaire{Simplification}
  2^k \cdot C\pars{3^{0}} + \sum\limits_{i=1}^{k}\pars{i3^i}
  \<=
  \commentaire{Simplification}
  2^k \cdot C\pars{1} + \sum\limits_{i=1}^{k}\pars{i3^i}
  \<=
  \commentaire{Valeur de base}
  2^k \cdot 0 + \sum\limits_{i=1}^{k}\pars{i3^i}
  \<=
  \commentaire{Simplification}
  \sum\limits_{i=1}^{k}\pars{i3^i}
  \<=
  \commentaire{>A CORRIGER À PARTIR DICI"!!!! Ajout de valeurs neutres}
